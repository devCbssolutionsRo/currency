...
class ResCurrencyRateProvider(models.Model):
    ...
    def _compute_name(self):
        for provider in self:
            name = False
            for service in self._fields['service'].selection:
                if provider.service == service[0]:
                    name = service[1]
                    break
            provider.name = name
    ...
    def _compute_available_currency_ids(self):
        ...
        for provider in self:
            supported_currencies_ids = Currency.search([('name','in',provider._get_supported_currencies())] ) 
            provider.available_currency_ids = [(6, 0, [x.id for x in supported_currencies_ids])] 
